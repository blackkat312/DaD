...
class Pregnancy_Events:
    ...
    def handle_having_kits(cat, clan):
        ...
        All_Sterile = True
        if second_parent and second_parent[0] == "Surrogate" and 'sterility' not in cat.permanent_condition:
            All_Sterile = False
            ...
            for x in second_parent:
                if 'sterility' not in x.permanent_condition:
                    All_Sterile = False

        if not int(random.random() * chance):
            # If you've reached here - congrats, kits!
            if kits_are_adopted or 'sterility' in cat.permanent_condition or (second_parent and All_Sterile):
    ...
    def handle_zero_moon_pregnant(cat: Cat, other_cat=None, surrogate=False, clan=game.clan):
        ...
        if cat and (cat.no_kits or "sterility" in cat.permanent_condition):
            return
        if other_cat:
            for x in other_cat:
                if x.no_kits or "sterility" in x.permanent_condition:
                    return
        ...
        if clan.clan_settings["same sex birth"] and not (not other_cat and random.randint(0,1)):
            ...
            if len(cat.illnesses) > 0:
                for illness in cat.illnesses:
                    if illness in Pregnancy_Events.fever_coat_illnesses and random.random() < 0.25:
                        fever = True
            ...
            if (not other_cat or surrogate) and 'Y' in cat.genotype.sexgene and (cat.gender != "intersex" or randint(1, 5) > 3):
                ...
                possible_affair_partners = [i for i in unknowns if
                                        ...
                                        and 'sterility' not in cat.permanent_condition
                                        and 'sterility' not in i.permanent_condition
                                        and (i.gender != "intersex" or randint(1, 5) > 2)
                    ...
                    if cat_type != 'Clancat':
                        if game.clan.clan_settings["same sex birth"]:
                            ...
                            if 'Y' in cat.genotype.sexgene:
                                ...
                                mate_gender = "masc"
                        while not outside_parent or 'sterility' in outside_parent.permanent_condition:
    ...
    def handle_one_moon_pregnant(cat: Cat, clan=game.clan):
        ...
        if len(cat.illnesses) > 0 and not fever:
            for illness in cat.illnesses:
                if illness in Pregnancy_Events.fever_coat_illnesses and random.random() < 0.33:
        ...
        if not other_cat:
            ...
            if(random.random() < 0.75 or len(possible_affair_partners) < 1):
                ...
                for i in range(0, nr_of_parents):
                    ...
                    if cat_type != 'Clancat':
                        out_par = None
                        while not out_par or 'sterility' in out_par.permanent_condition:
    ...
    def check_if_can_have_kits(cat, single_parentage, allow_affair):
        ...
        if 'sterility' in cat.permanent_condition:
            return False
        ...
        for check_cat in all_cats:
            for x in check_cat.relationships.values():
                ...
                if (x.romantic_love + x.platonic_like + x.admiration + x.trust + x.comfortable - x.dislike - x.jealousy) > 20:
                    if Pregnancy_Events.check_if_can_have_kits(check_cand, True, True) and not check_cand.mate and xor('Y' in check_cand.genotype.sexgene, 'Y' in cat.genotype.sexgene) and 'sterility' not in check_cand.permanent_condition:
                        possible = True
            ...
            if not cat:
                ...
                if not blood_parent:
                    ...
                    for i in range(0, nr_of_parents):
                        ...
                        while not blood_par2 or 'sterility' in blood_par2.permanent_condition:
    ...
    def get_amount_of_kits(cat, clan):
        ...
        if(clan.clan_settings['modded_kits']):
            ...
            six_kits = [min_kits + 5] * game.config["pregnancy"]["six_kit_possibility"][cat.age.value]
            seven_kits = [min_kits + 6] * game.config["pregnancy"]["seven_kit_possibility"][cat.age.value]
            eight_kits = [min_kits + 7] * game.config["pregnancy"]["eight_kit_possibility"][cat.age.value]
            nine_kits = [min_kits + 8] * game.config["pregnancy"]["nine_kit_possibility"][cat.age.value]
            ten_kits = [min_kits + 9] * game.config["pregnancy"]["ten_kit_possibility"][cat.age.value]
            eleven_kits = [min_kits + 10] * game.config["pregnancy"]["eleven_kit_possibility"][cat.age.value]
            twelve_kits = [min_kits + 11] * game.config["pregnancy"]["twelve_kit_possibility"][cat.age.value]
            thirteen_kits = [min_kits + 12] * game.config["pregnancy"]["thirteen_kit_possibility"][cat.age.value]
            fourteen_kits = [min_kits + 13] * game.config["pregnancy"]["fourteen_kit_possibility"][cat.age.value]
            fifteen_kits = [min_kits + 14] * game.config["pregnancy"]["fifteen_kit_possibility"][cat.age.value]
            sixteen_kits = [min_kits + 15] * game.config["pregnancy"]["sixteen_kit_possibility"][cat.age.value]
            seventeen_kits = [min_kits + 16] * game.config["pregnancy"]["seventeen_kit_possibility"][cat.age.value]
            eighteen_kits = [min_kits + 17] * game.config["pregnancy"]["eighteen_kit_possibility"][cat.age.value]
