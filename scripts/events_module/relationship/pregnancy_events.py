...
class Pregnancy_Events:
    ...
    fever_coat_illnesses = [
        "greencough", "kittencough", "an infected wound", "carrionplace disease", "whitecough", "silvercough",
        "yellowcough", "a festering wound", "heat stroke", "heat exhaustion", "ear infection"
    ]
    ...
    def handle_having_kits(cat, clan):
        ...
        if cat.ID in clan.pregnancy_data:
            ...
            if moons >= 2:
                if "pregnant" in cat.injuries:
                    ...
                    ids = []
                    second_parent = []
                    for entry in clan.pregnancy_data[cat.ID]["second_parent"]:
                        ids.append(entry)
                    for entry in ids:
                        second_parent.append(Cat.all_cats.get(entry))
        ...
        all_neutered = True
        if second_parent and second_parent[0] == "Surrogate" and not cat.neutered:
            all_neutered = False
        ...
        if not int(random.random() * chance):
            ...
            elif kits_are_adopted or cat.neutered or (second_parent and all_neutered):
                Pregnancy_Events.handle_adoption(cat, second_parent, clan)
            else:
                surrogate = False
    ...
    def handle_adoption(cat: Cat, other_cat=None, clan=game.clan):
        ...
        if cat.ID in clan.pregnancy_data and "false pregnancy" not in cat.injuries:
            return
        ...
        if clan.game_mode != "classic" and not cat.dead and "false pregnancy" in cat.injuries:
            cat.injuries.pop("false pregnancy")

        print_event = " ".join(event_list)
        print_event = print_event.replace("{insert}", insert)

        print_event = event_text_adjust(Cat, print_event, main_cat=cat, random_cat=random_choice, clan=clan)
    ...
    def handle_zero_moon_false(cat: Cat, other_cat=None, clan=game.clan):
        ...
        if other_cat:
            other_cat_copy = []
            for x in other_cat:
                if not (x.dead or (x.outside and x.status not in ['kittypet', 'loner',
                                                                  'rogue']) or x.birth_cooldown > 0 or x.no_kits):
        ...
        if not clan.clan_settings["same sex birth"] and (cat.genotype.gender == "tom" or (cat.genotype.gender == "intersex" and "excess testosterone" not in cat.permanent_condition and "aneuploidy" not in cat.permanent_condition and "chimerism" not in cat.permanent_condition) or cat.neutered):
            return
        ...
        if clan.game_mode != "classic":
            ...
            cat.get_injured("false pregnancy", severity=severity[0])
    ...
    def handle_zero_moon_pregnant(cat: Cat, other_cat=None, surrogate=False, clan=game.clan):
        ...
        if cat and (cat.no_kits or cat.neutered):
            return
        if other_cat:
            for x in other_cat:
                if x.no_kits or x.neutered:
                    return
        ...
        if clan.clan_settings['same sex birth'] and not (not other_cat and random.randint(0,1)):
            ...
            if len(cat.illnesses) > 0:
                for illness in cat.illnesses:
                    if illness in fever_coat_illnesses and random.random() < 0.25:
                        fever = True
            ...
            if (not other_cat or surrogate) and (cat.gender == "tom" or (cat.gender == "intersex" and ("testosterone deficiency" in cat.permanent_condition or ("chimerism" in cat.permanent_condition and randint(1, 2) == 1) or "mosaicism" in cat.permanent_condition))):
                ...
                possible_affair_partners = [i for i in unknowns if
                                        ...
                                        or (cat.gender != i.gender
                                        or ((cat.gender == "intersex" or i.gender == "intersex")
                                        and Pregnancy_Events.check_intersex_parents(cat, i, False)[0]
                                        and not Pregnancy_Events.check_intersex_parents(cat, i, False)[1])))
                ...
                if surrogate:
                    ...
                    outside_parent = None
                    if cat_type != 'Clancat':
                        if game.clan.clan_settings["same sex birth"]:
                            ...
                            mate_gender = random.choice(["fem", "masc"])
                        while not outside_parent or outside_parent.neutered:
            ...
            if second_parent:
                for x in second_parent:
                    if (cat.gender == "intersex" or x.gender == "intersex") and not (Pregnancy_Events.check_intersex_parents(cat, x, False)[0] and not Pregnancy_Events.check_intersex_parents(cat, x, False)[1]):
                        skip = True
                        break
                    if cat.gender == "tom" and x.gender == "molly":
                        pregnant_cat = x
    ...
    def handle_one_moon_pregnant(cat: Cat, clan=game.clan):
        ...
        if "false pregnancy" in cat.injuries:
            if cat.gender == "tom":
                failure = game.config['pregnancy']["failed_adopt"]["tom"]
            elif cat.gender == "molly":
                failure = game.config['pregnancy']["failed_adopt"]["molly"]
            else:
                failure = game.config['pregnancy']["failed_adopt"]["intersex"]

            if randint(1, failure) == 1:
                text = choice(Pregnancy_Events.PREGNANT_STRINGS["empty_nest"])
                cat.injuries.pop("false pregnancy")
        ...
        if clan.game_mode != "classic":
            try:
                ...
                if "false pregnancy" not in cat.injuries:
    ...
    def handle_two_moon_pregnant(cat: Cat, clan=game.clan):
        ...
        elif (len(cat.mate) > 0 and affair_partners) or (affair_partners and len(RandomAffair.mate) > 0 and cat.ID not in RandomAffair.mate and not RandomAffair.dead):
    ...
    def check_second_parent(
        ...
        # Checks for second parent alone:
            ...
            second_parent_copy = []
            must_be_adopted = False

            for x in second_parent:
                if (cat.gender != x.gender or ((cat.gender == "intersex" or x.gender == "intersex") and Pregnancy_Events.check_intersex_parents(cat, x, same_sex_adoption)[0])) or same_sex_birth:
                    if Pregnancy_Events.check_intersex_parents(cat, x, same_sex_adoption)[0] and Pregnancy_Events.check_intersex_parents(cat, x, same_sex_adoption)[1]:
                        must_be_adopted = True
            ...
            if not chance_love_affair or not int(random.random() * chance_love_affair):
                if (samesex
                        or (cat.gender != highest_romantic_relation.cat_to.gender
                            or ((cat.gender == "intersex" or highest_romantic_relation.cat_to.gender == "intersex")
                                and Pregnancy_Events.check_intersex_parents(cat, highest_romantic_relation.cat_to, False)[0]
                                and not Pregnancy_Events.check_intersex_parents(cat, highest_romantic_relation.cat_to, False)[1]))):
    ...
    def get_amount_of_kits(cat, clan):
        ...
        if(clan.clan_settings['modded_kits']):
            ...
            twelve_kits = [min_kits + 11] * game.config["pregnancy"]["twelve_kit_possibility"][cat.age]
            thirteen_kits = [min_kits + 12] * game.config["pregnancy"]["thirteen_kit_possibility"][cat.age]
            fourteen_kits = [min_kits + 13] * game.config["pregnancy"]["fourteen_kit_possibility"][cat.age]
            fifteen_kits = [min_kits + 14] * game.config["pregnancy"]["fifteen_kit_possibility"][cat.age]
            sixteen_kits = [min_kits + 15] * game.config["pregnancy"]["sixteen_kit_possibility"][cat.age]
            ...
            amount = choice(
                ...
                sixteen_kits + seventeen_kits + eighteen_kits + max_kits
            )
