...
class HandleShortEvents:
    ...
    def handle_transition(self):
        ...
        if possible_genders:
            ...
            self.main_cat.genderalign = "intersex " if self.main_cat.genotype.sex == "intersex" else "" + new_gender.replace("molly", "molly").replace("tom", "tom").replace("nonbinary", "non-binary")
                ...
                if "lost" in self.chosen_event.tags:
                    ...
                    if tnr and 'TNR' not in kitty.pelt.scars:
                        if kitty.moons > 3:
                            ...
                            kitty.get_permanent_condition("sterility", False, custom_reveal=2 if 'pregnant' in kitty.injuries else None)
                        if kitty.moons < 4:
                            kitty.status = 'kittypet'
                            kitty.get_permanent_condition("sterility", False, event_triggered=True, custom_reveal=randint(4, 6))
...
INJURY_GROUPS = {
    ...
    "broken_injury": ["broken jaw", "broken bone", "broken back"],
}
