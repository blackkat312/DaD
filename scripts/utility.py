...
def create_bio_parents(Cat, cat_type, flip=None, second_parent=True):
    if not flip:
        flip = choice(["fem", "masc"])
    ...
    blood_parent = create_new_cat(Cat,
                                    ...
                                    outside=True,
                                    is_parent=True)[0]
    while 'sterility' in blood_parent.permanent_condition:
        if(blood_parent):
    ...
    if second_parent:
        ...
        blood_parent2 = create_new_cat(Cat,
                                    ...
                                    outside=True,
                                    is_parent=True)[0]
        while 'sterility' in blood_parent2.permanent_condition:
    ...
    if not chosen_cat:
        ...
        if status in ["kitten", "newborn"]:
            if game.clan.clan_settings["same sex birth"]:
                ...
                if 'Y' in parent1.genotype.sexgene:
                    sex = "fem"
                else:
                    sex = "masc"
            generated_parents = create_bio_parents(Cat, cat_type, flip=sex, second_parent=not parent1)
            if not parent1:
    ...
    for index in range(number_of_cats):
        ...
        if other_clan or kit or litter or age < 12 and not (loner or kittypet):
            ...
            if kittypet:
                ...
                kittyname = True
            elif loner and randint(1, 5) > 3:  # try to give name from full loner name list
                name = choice(names.names_dict["loner_names"])
                lonername = True
            else:
                name = choice(
            ...
            # python doesn't want to work with me TwT
            if kittyname:
                if randint(1, 5) > 3:
                    accessory = choice(["CRIMSON", "BLUE", "YELLOW", "CYAN", "RED", "LIME", "GREEN", "RAINBOW", "BLACK",
                                        "SPIKES", "WHITE", "PINK", "PURPLE", "MULTI", "INDIGO", "CRIMSONBELL",
                                        "BLUEBELL", "YELLOWBELL", "CYANBELL", "REDBELL", "LIMEBELL", "GREENBELL",
                                        "RAINBOWBELL", "BLACKBELL", "SPIKESBELL", "WHITEBELL", "PINKBELL", "PURPLEBELL",
                                        "MULTIBELL", "INDIGOBELL", "CRIMSONBOW", "BLUEBOW", "YELLOWBOW", "CYANBOW",
                                        "REDBOW", "LIMEBOW", "GREENBOW", "RAINBOWBOW", "BLACKBOW", "SPIKESBOW",
                                        "WHITEBOW", "PINKBOW", "PURPLEBOW", "MULTIBOW", "INDIGOBOW", "CRIMSONNYLON",
                                        "BLUENYLON", "YELLOWNYLON", "CYANNYLON", "REDNYLON", "LIMENYLON", "GREENNYLON",
                                        "RAINBOWNYLON", "BLACKNYLON", "SPIKESNYLON", "WHITENYLON", "PINKNYLON",
                                        "PURPLENYLON", "MULTINYLON", "INDIGONYLON"])
            elif lonername:
                if randint(1, 5) == 1:
                    accessory = choice(["MAPLE LEAF", "HOLLY", "BLUE BERRIES", "FORGET ME NOTS", "RYE STALK", "CATTAIL",
                                        ...
                                        "DRY LAURELS", "RED FEATHERS", "BLUE FEATHERS", "JAY FEATHERS", "GULL FEATHERS",
        ...
        if accessory and not (("NOTAIL" in new_cat.pelt.scars or "HALFTAIL" in new_cat.pelt.scars or (new_cat.phenotype.bobtailnr > 0 and new_cat.phenotype.bobtailnr < 5)) and accessory in ["RED FEATHERS", "BLUE FEATHERS", "JAY FEATHERS", "GULL FEATHERS", "SPARROW FEATHERS", "CLOVER", "DAISY"]):
        ...
        if not is_parent and game.clan.clan_settings['tnr_mode'] and age > 5 and not new_cat.genotype.sexgene == ["Y"]:
            kittypet_n = game.config['tnr_mode']['kittypet_neuter']
            loner_n = game.config['tnr_mode']['loner_tnr']
            if kittypet and random() < kittypet_n:
                new_cat.get_permanent_condition("sterility", False)
...
def process_text(text, cat_dict, raise_exception=False):
    ...
    # THE WALL
    adjust_text.replace("medicine cat", "healer")

    adjust_text.replace("medicine den", "healers' den")
    adjust_text.replace("medicine cat den", "healers' den")
    adjust_text.replace("medicine cats den", "healers' den")
    adjust_text.replace("medicine cat's den", "healers' den")
    adjust_text.replace("medicine cats' den", "healers' den")

    adjust_text.replace("healer den", "healers' den")
    adjust_text.replace("healers den", "healers' den")
    adjust_text.replace("healer's den", "healers' den")

    adjust_text.replace("apprentice den", "apprentices' den")
    adjust_text.replace("apprentices den", "apprentices' den")
    adjust_text.replace("apprentice's den", "apprentices' den")

    adjust_text.replace("warrior den", "warriors' den")
    adjust_text.replace("warriors den", "warriors' den")
    adjust_text.replace("warrior's den", "warriors' den")

    adjust_text.replace("elder den", "elders' den")
    adjust_text.replace("elders den", "elders' den")
    adjust_text.replace("elder's den", "elders' den")

    adjust_text.replace("leader den", "leader's den")
    adjust_text.replace("leaders den", "leader's den")
    adjust_text.replace("leaders' den", "leader's den")

    adjust_text.replace("mossball", "moss-ball")
    adjust_text.replace("moss ball", "moss-ball")
...
def generate_sprite(
    ...
    def draw_sprite(genotype, phenotype, cat_sprite, somatic = False):
        ...
        if somatic:
            ...
            phenotype.PhenotypeOutput(gender=genotype.sex, sex=genotype.sex, pattern=genotype.white_pattern)
        ...
        def GenSprite(genotype, phenotype, sprite_age, merle=False):
            ...
            def TabbyBase(whichcolour, whichbase, cat_unders, special = None):
                ...
                if not is_red and 'silver' not in whichbase:
                    ...
                    whichmain.blit(sprites.sprites[whichbase], (0, 0))
                if special !='copper' and sprite_age > 12 and (genotype.silver[0] == 'I' and genotype.corin[0] == 'fg' and (get_current_season() == 'Leaf-fall' or get_current_season() == 'Leaf-bare' or 'sterility' in cat.permanent_condition)):
            ...
            if not is_white:
                ...
                if(phenotype.patchmain != ""):
                    for pattern in genotype.tortiepattern:
                        ...
                        nonrevpattern = pattern.replace('rev', "")
                        tortpatches2.blit(sprites.sprites[f"tortiemask{nonrevpattern}" + cat_sprite], (0, 0))
                ...
                if(genotype.merlepattern != None and not merle):
                    for pattern in genotype.merlepattern:
                        ...
                        nonrevpattern = pattern.replace('rev', "")
                        merlepatches2.blit(sprites.sprites[f"tortiemask{nonrevpattern}" + cat_sprite], (0, 0))
