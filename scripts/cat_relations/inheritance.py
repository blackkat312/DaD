...
while setting mates and for the display of the family tree screen.
...
class Inheritance:
    ...
    def init_great_grandparents(self):
        ...
        for grand_parent_id, value in self.grand_parents.items():
            ...
            for grand_id in great_grandparents:
                ...
                if grand_id not in self.great_grandparents:
                    self.great_grandparents[grand_id] = {
                        "type": great_grand_type,
                        "additional": []
                    }
                    self.all_involved.append(grand_id)
                    self.all_but_cousins.append(grand_id)
                self.great_grandparents[grand_id]["additional"].append(
                    f"parent of {str(grandparent_cat.name)}"
                )
    ...
    def get_not_great_blood_grand_parents(self) -> list:
        ...
        return self.get_no_blood_relatives(self.great_grandparents)

    def get_great_grandparents(self) -> list:
        """Returns a list of id's which are great_grandparents to the cat, according to the inheritance hierarchy."""
        return self.get_blood_relatives(
            self.great_grandparents
    ...
    def get_exact_rel_type(self, cat_id):
        ...
        if cat_id in self.great_grandparents:
            all_relations.append(self.great_grandparents[cat_id])
        if cat_id in self.grand_kits:
