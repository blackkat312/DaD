...
class Cat:
    ...
    normal_traits = [
        ...
        "strange", "vengeful", "wise", "arrogant", "competitive", "grumpy", "cunning", "oblivious", "gloomy",
        "sincere", "flamboyant", "rebellious"
    ]

    kit_traits = [
        "unruly", "shy", "impulsive", "bullying", "attention-seeker", "charming", "fearless", "noisy", "skittish",
        "quiet", "self-conscious", "daydreamer", "sweet", "polite", "know-it-all", "bossy"
    ]
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.phenotype.PhenotypeOutput(self.genotype.white_pattern, gender=self.genotype.sex, sex=self.genotype.sex)
    ...
    def init_generate_cat(self, skill_dict):
        ...
        # GENDER IDENTITY
        if self.moons == 0 or self.age == CatAgeEnum.NEWBORN:
        ...
        if not theythemdefault and not (self.moons == 0 or self.age == CatAgeEnum.NEWBORN):
    ...
    def genetic_conditions(self):
        ...
        if "intersex" not in self.conditions_already_attempted:
            ...
            # Chimeras
            if self.genotype.chimera and self.genotype.sex != "intersex":
                ...
                self.conditions_already_attempted.append("intersex")
                self.pronouns = [self.default_pronouns[0].copy()]
                if not game.settings["they them default"] and self.age != CatAgeEnum.NEWBORN:
                    self.handle_pronouns()
            # Aneuploidy
            if aneuploidy and self.genotype.sex != "intersex":
                if self.gender == self.genderalign:
                    self.genderalign = "intersex"
                self.gender = "intersex"
                self.genotype.sex = "intersex"
                ...
                if not game.settings["they them default"] and self.age != CatAgeEnum.NEWBORN:
                    self.handle_pronouns()
            # XY mollies
            if (self.genotype.sexgene == ["o", "Y"] or self.genotype.sexgene == ["O", "Y"]) and self.genotype.sex == "molly":
        ...
        if "ocular albinism" not in self.conditions_already_attempted:
            if (
                    ...
                    and "albinism" not in self.conditions_already_attempted
            ):
        ...
        if "fully hairless" not in self.conditions_already_attempted:
            if self.phenotype.length == 'hairless':
                self.get_permanent_condition('fully hairless', born_with=True)
        ...
        if "partially hairless" not in self.conditions_already_attempted:
            if self.phenotype.length == 'fur-pointed' or 'patchy ' in self.phenotype.furtype:
                self.get_permanent_condition('partially hairless', born_with=True)
                self.conditions_already_attempted.append("partially hairless")

        if self.genotype.munch[0] == 'Mk':
            if "joint pain" not in self.conditions_already_attempted:
                if random() < 0.33:
                    self.get_permanent_condition('constant joint pain', born_with=True, custom_reveal=randint(24, 36))
                self.conditions_already_attempted.append("joint pain")
        ...
        if "cysts" not in self.conditions_already_attempted:
            if self.genotype.lykoi[0] == 'ly':
                self.get_permanent_condition('bumpy skin', born_with=True)
        ...
        for cat in Cat.all_cats.values():
            ...
            # it can fail to to minor grief. If they have a family relation, bypass the roll.
                ...
                minor_grief_messages = (
                    ...
                    "Lashes out at every cat who checks on {PRONOUN/m_c/object} after r_c's death",
                    ...
                    "Takes a part of r_c's nest to put in {PRONOUN/m_c/poss} own, clinging to the fading scent",
                    "Sleeps in r_c's nest tonight",
                    "Defensively states that {PRONOUN/m_c/subject} {VERB/m_c/do/does}n't need to be comforted over r_c's death",
                    "Wonders why StarClan had to take r_c so soon",
                    "Still needs r_c even though {PRONOUN/r_c/subject}{VERB/r_c/'re/'s} gone",
                    ...
                    "Is hiding {PRONOUN/m_c/poss} tears as {PRONOUN/m_c/subject} {VERB/m_c/comfort/comforts} {PRONOUN/m_c/poss} Clanmates over r_c's passing",
                )
                ...
                if body:
                    minor_grief_messages += (
                        ...
                        "Sleeps next to r_c for the entire vigil",
    ...
    def generate_lead_ceremony(self):
        ...
        if len(life_givers) < 3:
            amount = 3 - len(life_givers)
        ...
        if len(life_givers) < 4:
            unknown_blessing = True
        else:
            unknown_blessing = False
        extra_lives = str(4 - len(life_givers))
    ...
    def one_moon(self):
        ...
        if not self.prevent_trainee:
            ...
            if self.age == CatAgeEnum.KITTEN:
                join_chance += 100
                leave_chance -= 100
            elif self.age in [CatAgeEnum.SENIOR_ADULT, CatAgeEnum.SENIOR]:
                join_chance += 50
                leave_chance -= 50
            elif self.age == CatAgeEnum.ADOLESCENT:
                if self.moons < 9:
        ...
        if condition in ["budding spirit", "shattered soul"]:
            ...
            if template["other"] == "fictive" or template["other"] == "fuzztive":
                ...
                if canon_chance == 1:
                    ...
                    alter_personality = choice(self.normal_traits)
                ...
                if template["other"] in ["fictive", "fuzztive"]:
                    ...
                    if canon_chance == 1:
                        ...
                        if randint(1, 2) == 1:
                            ...
                            alter_name += "kit"
                            alter_personality = choice(kit_traits)
    ...
    def moon_skip_permanent_condition(self, condition):
        ...
        if condition in ["shattered soul", "budding spirit", "fractured spirit"]:
            ...
            for alter in self.alters:
                if ("recovering from birth" not in self.injuries and "false pregnancy" not in self.injuries and "pregnant" not in self.injuries and "turmoiled litter" not in self.illnesses) or (("recovering from birth" in self.injuries or "false pregnancy" in self.injuries or "pregnant" in self.injuries or "turmoiled litter" in self.illnesses) and alter["role"] != "little"):
                    can_front.append(alter["name"])
            self.front = choice(can_front)
            if self.moons > 11 and self.status not in ["apprentice", "medicine cat apprentice", "mediator apprentice"]:
    ...
    def congenital_condition(self, cat):
        ...
        count = 1
        genetics_exclusive = [
            'albinism', 'ocular albinism', "flat nose", 'manx syndrome', "fully hairless", "partially hairless", "bumpy skin"
        ]
        ...
        if self.alters:
            for alter in self.alters:
                ...
                if "personality" not in alter:
                    alter["personality"] = choice(self.normal_traits)
    ...
    def get_permanent_condition(self, name, born_with=False, event_triggered=False, starting_moon=0, custom_reveal=None):
        ...
        if name == "albinism" and not (self.genotype.pointgene[0] == 'c' or (self.genotype.chimera and self.genotype.chimerageno.pointgene[0] == 'c')):
            return
        elif name == "ocular albinism" and not (
                ('albino' in self.genotype.lefteyetype or 'albino' in self.genotype.righteyetype or (self.genotype.extraeye and 'albino' in self.genotype.extraeyetype) or self.genotype.pinkdilute[0] == 'dp')
                or (self.genotype.chimera and ('albino' in self.genotype.chimerageno.lefteyetype or 'albino' in self.genotype.chimerageno.righteyetype or (self.genotype.chimerageno.extraeye and 'albino' in self.genotype.chimerageno.extraeyetype) or self.genotype.chimerageno.pinkdilute[0] == 'dp'))
                or (self.genotype.somatic and self.genotype.somatic != "{}" and "face" in self.genotype.somatic["base"] and (self.genotype.somatic["allele"] == "c" or self.genotype.somatic["allele"] == "dp"))
                or (self.genotype.chimera and self.genotype.chimerageno.somatic and self.genotype.chimerageno.somatic != "{}" and "face" in self.genotype.chimerageno.somatic["base"] and (self.genotype.chimerageno.somatic["allele"] == "c" or self.genotype.chimerageno.somatic["allele"] == "dp"))
        ...
        if name == "fully hairless" and self.phenotype.length != 'hairless':
            return
        elif name == "partially hairless" and not (self.phenotype.length == 'fur-pointed' or 'patchy ' in self.phenotype.furtype):
            return
