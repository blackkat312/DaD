...
from scripts.game_structure.localization import load_lang_resource, get_new_pronouns
...
class Events:
    ...
    def handle_tnr_return(self):
        ...
        cat_IDs = []
        for cat in Cat.all_cats.values():
            TNRed = True if ('sterility' in cat.permanent_condition and 'TNR' in cat.pelt.scars and 
            game.clan.age - cat.permanent_condition['sterility']['moon_start'] == 1) else False
            if (cat.outside
            and cat.status
            not in [
                "kittypet",
        ...
        for cat in eligible_cats:
            ...
            for x in additional:
                if x in Cat.all_cats:
                    ...
                    Cat.all_cats[x].get_permanent_condition("sterility", False, custom_reveal=4-Cat.all_cats[x].moons)
    ...
    def handle_colour_changes(self, cat):
        ...
        if ('full white' in cat.genotype.white_pattern and not has_break_white) or cat.genotype.pointgene[0] == 'c' or 'o' not in cat.genotype.sexgene:
            return
    ...
    def perform_ceremonies(self, cat):
        ...
        if game.clan.deputy:
            if (
                ...
                text = ""
                if game.clan.deputy.personality.trait == "bloodthirsty" and random.randint(1, 3) == 1:
    ...
    def coming_out(self, cat):
        ...
        '''
        Base game code (may be combined at some point, but uh... that's a lot more work):
        if cat.age.is_baby():
            return
        ...
        if cat.genderalign == cat.gender:
            ...
            cat.pronouns = get_new_pronouns(cat.genderalign)
            ...
            if cat.genderalign == "trans molly":
                ...
                trans += "a tom"
            elif cat.genderalign == "demimolly":
                trans += "a demimolly"
            ...
            if cat.gender != "intersex" and cat.genderalign not in ["trans molly", "trans tom"]:
                text = random.choice(
                    [
                        ...
                        f"For a long time, the only term {cat.name} knew to describe themself was \"{cat.gender}\" - and it never really felt like it fit. However, when they find the term {cat.genderalign}, they feel so much better calling themself that instead of {gender}."
                    ]
                )
            else:
        ...
        if (
            ...
            possible_deputies = list(
                filter(
                    ...
                    and x.status == "warrior"
                     and ([i for i in x.former_apprentices if 'apprentice' not in Cat.all_cats.get(i).status]),
                    Cat.all_cats_list,
                )
            )
            ...
            if possible_deputies:
                ...
                if leader_status == "here" and deputy_status == "not_here":
                    if random_cat.personality.trait == "bloodthirsty" and random.randint(1, 3) == 1:
